...
class CoordSingle(BaseData):
    """
    Single trajectory coordinate data handler for JADE-NAMD simulations.
    
    Reads and processes atomic coordinate information from traj_time.out files,
    providing methods for geometry analysis and visualization.
    """
    
    def __init__(self, path: str, max_i_time: int, type: str = "folder"):
        """
        Initialize CoordSingle.
        
        Args:
            path: Path to data source (folder with traj_time.out or CSV/Pickle file)
        ...
        if type == "folder":
            ...
            data = self._build_dataframe(sym_frames, coord_frames, time_frames)
        elif type == "csv":
        ...
        super().__init__(data, max_i_time, "coordinate")
